...
P_VALUES = [0.0, .02]  # measurement rate threshold
L_MAX = 20  # max layers
N_VALUES = [6]  # system sizes (even, 8–16)
N_REALIZATIONS = 200  # single-shot mode: independent circuits per (N, p, L)
...
L_VALUES: list[int] | None = [2,5,10,20,30]  # explicit L list; overrides L_MAX + RECORD_EVERY when set
...
def estimate_hqc_cost(mode: str = "single_shot", verbose: bool = True) -> float:
    ...
    for W in N_VALUES:
        for p in P_VALUES:
            for L in L_values:
                ...
                hqc_point = (5 + (N1q + 10 * N2q + 5 * Nm) / 5000 * Ns) * n_circuits_point
                total_hqc += hqc_point
    ...
    if verbose:
        ...
        print(f"{'N':>4} {'p':>5} {'L':>4} {'N1q':>6} {'N2q':>6} {'Nm':>5} {'#circs':>7} {'Ns':>4} {'HQC':>10}")
        print("-" * 62)
        for W, p, L, N1q, N2q, Nm, n_c, Ns, hqc in rows:
            print(f"{W:>4} {p:>5.2f} {L:>4} {N1q:>6} {N2q:>6} {Nm:>5} {n_c:>7} {Ns:>4} {hqc:>10.1f}")
        print("-" * 62)
...
def _merged_to_all_stats(merged: dict) -> tuple:
    ...
    all_stats = {N: compute_F(dict(raw_by_N[N])) for N in N_values}
