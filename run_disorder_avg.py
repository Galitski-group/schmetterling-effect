...
_src = pathlib.Path(__file__).parent / "src" / "__main__.py"
...
P_VALUES = [0.0, 0.01, 0.04, 0.05]  # measurement rate threshold
L_MAX = 30  # max layers
N_VALUES = [8]  # system sizes (even, 8–16)
N_REALIZATIONS = 500  # single-shot mode: independent circuits per (N, p, L)
...
RECORD_EVERY = 2  # record at L = 5, 10, 15, 20
...
def _build_backend():
    """Construct and return the backend object from DEVICE_NAME."""
